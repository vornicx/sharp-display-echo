...
const TIPOS: { value: string; label: string; unit: string }[] = [
  { value: "Cera",         label: "Cera",         unit: "kg"  },
  { value: "Agua",         label: "Agua",         unit: "L"   },
  { value: "Electricidad", label: "Electricidad", unit: "kWh" },
  { value: "Gasoil",       label: "Gasoil",       unit: "L"   },
  { value: "Fungicida",    label: "Fungicida",    unit: "kg"  },
];
...
const defaultUnit = (tipo: string) =>
  TIPOS.find((t) => t.value === tipo)?.unit ?? "kg";
...
function buildKpis(rows: Coste[]) {
  const sum = (tipo: string) =>
    rows.filter((r) => r.tipo === tipo).reduce((a, b) => a + b.cantidad, 0);
  const totalEur = rows.reduce((a, b) => a + b.cantidad * b.coste_unitario, 0);
  return {
    cera: sum("Cera"),
    agua: sum("Agua"),
...
const ConsumoCostes = () => {
  ...
  useEffect(() => {
    document.title = `Consumos — Lasarte SAT`;
  }, []);
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <KPICard
            label="Cera"
            value={kpisDay.cera.toLocaleString("es-ES", { maximumFractionDigits: 1 })}
            unit="kg"
            tone="warning"
          ...
          <KPICard
            label="Agua"
            value={kpisDay.agua.toLocaleString("es-ES", { maximumFractionDigits: 0 })}
            unit="L"
            tone="info"
          ...
          <KPICard
            label="Electricidad"
            value={kpisDay.electricidad.toLocaleString("es-ES", { maximumFractionDigits: 0 })}
            unit="kWh"
            tone="warning"
          ...
          <KPICard
            label="Gasoil"
            value={kpisDay.gasoil.toLocaleString("es-ES", { maximumFractionDigits: 1 })}
            unit="L"
            tone="default"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-muted-foreground font-medium">
              Cantidad ({defaultUnit(form.tipo)})
            </label>
            <Input
              type="number"
              placeholder="0"
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input
              ...
              onChange={(e) => setForm((f) => ({ ...f, coste_unitario: e.target.value }))}
              className="font-mono"
            />
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {dayRows.map((r) => (
                  <TableRow key={r.id} className="group">
                    <TableCell>
                      <Select
                        value={r.zona_id}
                        onValueChange={(v) => updateField(r.id, { zona_id: v })}
                      >
                    ...
                    <TableCell>
                      <span className={cn(
                        ...
                        TIPO_COLORS[r.tipo] ?? "bg-muted text-foreground"
                      )}>
                        {r.tipo}
                      </span>
                    </TableCell>
                    <TableCell className="text-right">
                      <Input
                        type="number"
                        value={r.cantidad}
                        onChange={(e) => updateField(r.id, { cantidad: Number(e.target.value) || 0 })}
                        className="w-[90px] text-right font-mono h-8 text-sm ml-auto"
                      />
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <Input
                        ...
                        onChange={(e) => updateField(r.id, { coste_unitario: Number(e.target.value) || 0 })}
                        className="w-[90px] text-right font-mono h-8 text-sm ml-auto"
                      />
                    </TableCell>
                    <TableCell className="text-right font-mono font-semibold text-sm">
                      {(r.cantidad * r.coste_unitario).toLocaleString("es-ES", {
                ...
                <TableRow className="border-t-2 border-border bg-muted/30">
                  <TableCell colSpan={5} className="text-sm font-semibold text-right pr-4">
                    Total día
                  </TableCell>
                  <TableCell className="text-right font-mono font-bold text-foreground">
                    {kpisDay.totalEur.toLocaleString("es-ES", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })} €
                  </TableCell>
