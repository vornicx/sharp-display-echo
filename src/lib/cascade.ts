 ...
 *   4. − (Palets dados de alta − Inventario_final_día_anterior)
...
export function computeCascade(i: CascadeInputs): CascadeOutput {
  ...
  const inv = invRaw; // inventario final del día actual (se resta por separado)
  const paletsNetos = palets - pendAnt; // palets netos = dados de alta − inventario final día anterior
